...
import type { IMessageRepository, IPhraseCandidatesRepo, PhraseCandidateRow, Message } from '../../src/storage/db.js';
...
function makeMiner(overrides: Partial<{
...
}> = {}): { miner: PhraseMiner; db: DatabaseSync; repo: IPhraseCandidatesRepo; messages: IMessageRepository; claude: IClaudeClient } {
  const db = overrides.db ?? makeDb();
  const messages = overrides.messages ?? makeMessageRepo();
  const claude = overrides.claude ?? makeClaude();
  const repo = overrides.phraseCandidates ?? new PhraseCandidatesRepository(db);
  ...
  return { miner, db, repo, messages, claude };
}
  ...
  it('skips candidates below threshold', async () => {
    ...
    db.prepare(`
      INSERT INTO phrase_candidates
        (group_id, content, gram_len, count, contexts, last_inference_count, meaning, is_jargon, promoted, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, 0, NULL, 0, 0, ?, ?)
    `).run('g1', '随便一句xx', 2, 2, '[]', nowSec, nowSec);
...
describe('PhraseMiner edge cases', () => {
  ...
  it('caps context list at MAX_CONTEXTS via repo', () => {
    const msgs = makeMessages(
