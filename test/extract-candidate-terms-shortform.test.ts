...
describe('extractCandidateTerms shortform expansion', () => {
  ...
  beforeEach(() => {
    ...
    db.learnedFacts.insert({
      groupId: 'g1',
      topic: 'opus-classified:fandom:羊宫妓那',
      fact: '羊宫妓那是BanG Dream的偶像角色',
      canonicalForm: '羊宫妓那',
      personaForm: null,
      sourceUserId: null,
  ...
  it('A1: ygfn 是谁 -> candidates includes 羊宫妓那 and findActiveByTopicTerm returns fact', () => {
    const cands = extractCandidateTerms('ygfn 是谁', 'g1', db.memeGraph);
    expect(cands).toContain('羊宫妓那');
    const facts = db.learnedFacts.findActiveByTopicTerm('g1', '羊宫妓那');
    expect(facts.length).toBeGreaterThanOrEqual(1);
  });

  // A2: CJK path unbroken
  it('A2: 羊宫妓那是谁 -> candidates[0] === 羊宫妓那 (C-1 path)', () => {
    const cands = extractCandidateTerms('羊宫妓那是谁', 'g1', db.memeGraph);
    expect(cands[0]).toBe('羊宫妓那');
  });

  // A3-edge-1: bare ygfn expands
  it('A3-edge-1: ygfn (bare) -> candidates includes 羊宫妓那', () => {
    const cands = extractCandidateTerms('ygfn', 'g1', db.memeGraph);
    expect(cands).toContain('羊宫妓那');
  });

  // A3-edge-2: all-caps YGFN -> toLowerCase normalization hits
  it('A3-edge-2: YGFN (all-caps) -> candidates includes 羊宫妓那', () => {
    const cands = extractCandidateTerms('YGFN', 'g1', db.memeGraph);
    expect(cands).toContain('羊宫妓那');
  });

  // A3-edge-3: ygfn with question tail
  it('A3-edge-3: ygfn 是谁 (with tail) -> includes 羊宫妓那 and ygfn', () => {
    const cands = extractCandidateTerms('ygfn 是谁', 'g1', db.memeGraph);
    expect(cands).toContain('羊宫妓那');
  ...
  it('A3-edge-8: 如何评价ygfn -> candidates includes 羊宫妓那', () => {
  ...
  it('case-mixed: Ygfn -> candidates includes 羊宫妓那', () => {
    const cands = extractCandidateTerms('Ygfn', 'g1', db.memeGraph);
    expect(cands).toContain('羊宫妓那');
  });
  ...
  it('DESIGN row 19: Ygfn mixed-case -> expands to 羊宫妓那', () => {
    const cands = extractCandidateTerms('Ygfn', 'g1', db.memeGraph);
    expect(cands).toContain('羊宫妓那');
  });
  ...
  it('expansion inserts CJK canonical before raw Latin token in candidate order', () => {
    ...
    const cjkIdx = cands.indexOf('羊宫妓那');
  ...
  it('DESIGN row 3: bare ygfn -> exact candidates [羊宫妓那, ygfn]', () => {
    const cands = extractCandidateTerms('ygfn', 'g1', db.memeGraph);
    expect(cands[0]).toBe('羊宫妓那');
