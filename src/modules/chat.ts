...
// First-person self-talk (我得/我要/我想 + bare verb) excluded via negative
// lookbehind on bare-verb sub-clauses — _generateDeflection now routes through
// LLM which amplifies task-FP into rude output, so FP rate matters.
// First-person self-talk (我+bare verb, no modal required) also excluded via
// address-prefix-aware nested lookbehind — bypass fires when 帮/给/替/让/教 precedes 我.
// Third-person narration (他/她/它/他们/她们/它们 + optional modal + bare verb)
// also excluded — peer-chat about a third party doing labor is not a task on
// the bot. NO [帮给替让教] carve-out for third-person (conservative bias —
// "帮他写" is rare, FP cost of wrongful deflection on third-person narration
// outweighs the rare positive). See PR for scope boundary on adverb-context
// (`先生成…` style narration) — out of scope; PR #128 send-side guard catches
// the residual.
export const TASK_REQUEST =
  /(?:帮我(?:写|编|做|生成|翻译|画|背|算|总结|实现)|给我(?:写|编|做|生成|翻译|画|作|算|总结)|替我(?:写|编|做|生成|翻译|画|背|算|总结)|让你(?:写|编|做|画|背|翻译|接)|让我(?:写|编|做|画|翻译)|麻烦你?(?:写|做|画|翻译|算|总结)|能不能(?:帮[我你]?|给[我你]?)?(?:写|编|做|画|翻译|算|总结)|求(?:你|求)(?:写|画|翻译)|你来(?:写|做|画|背|翻译)|你去(?:写|画|翻译)|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)写[个一](?:首|段|篇|下|个)?(?!.{0,2}(?:啥|什么))|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)写一下|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)编[个一](?:首|段|篇|下|个)?|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)编一下|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)生成(?:一个|一下|一段|一张|[个一])|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)翻译(?:一下|这[段句篇个]|下面)|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)画[个一](?:张|幅|下|段)?|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)作一首|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)推荐(?:一下|个|一个|一下什么|下什么|什么好)|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)念一?段|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)背一段|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)搞(?:一个|个)(?!.{0,3}(?:人|东西|意思|啥|什么|事))|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)整[个一](?:笑话|段子)|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)做一个(?!.{0,3}(?:人|梦|事))|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)算一下|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)算算(?!.{0,3}(?:看|了|就))|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)总结(?:一下|[个一]下)|接下一?句|后面[几一]?句|后面是.{0,10}[什么啥]|续[一下]|(?:让你|你来|教你|你要)接龙|(?:诗词|名字|课文|长文|古诗)接龙|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)继续[背念说]|往[下后]接|再来[一几]段|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)背[一下出来](?!.{0,3}(?:了|过))|让你接|你要接|现在你(?:需要|要)接|前面是.{0,5}[什么啥]|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)教.{0,5}(?:swift|python|js|java|代码|编程|算法|怎么写)|怎么(?:写|实现)代码|帮我(?:写|实现)代码|代码怎么|(?<!(?<![帮给替让教])(?:我|我们)\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)?\s*)(?<!(?:他|她|它)(?:们)?\s*(?:要|得|想|打算|准备|需要|必须|该|应该)?\s*)教教.{0,5}(?:怎么)?(?:写|做|实现)|(?:transformer|optimizer|激活函数|神经网络|attention|算法|API).{0,10}(?:怎么|如何|原理))/;

export const TASK_DEFLECTIONS = [
  '我不会', '你自己写', '想屁吃', '懒得', '不想',
  '写不出来', '不擅长这个', '？', '我又不是工具人', '想得美', '哈哈你自己搞',
  '自己背', '懒得做', '你恩师是谁', '哈哈谁背这个', '你做梦', '无语', '不接', '想多了',
];
...
export const MEMORY_INJECT_DEFLECTIONS = [
  '记不住', '我又不是你电脑', '啥呢', '谁关心', '懒得记', '嗯', '?', '好好好', '啥啊', '不想记',
];

export const CURSE_DEFLECTIONS = [
  '傻逼', '你有病吧', '滚', '煞笔', '有毛病',
  '没完了是吧', '烦死了 闭嘴', '一天天的', '真没完了', '别闹了', '笑死 算了',
];
...
export const DEFLECT_SITUATIONS: Record<DeflectCategory, string> = {
  ...
  recite: '有人让你背长诗/续写课文/接续某段长内容。你不会这种东西，直接怼回去或装懵。',
  curse: '这个人已经反复在逗你/挑衅你第三次以上了。你现在真的烦了，可以带脾气说重一点（烦死了/滚/真烦/别闹了/懒得）。但不要人身攻击到超出骂人的程度。',
...
export const DEFLECT_FALLBACKS: Record<DeflectCategory, string[]> = {
  ...
  at_only: ['?', '嗯?', '叫我干嘛', '说话呀', '艾特我干啥', '?怎么了', '什么事', '在', '咋了'],
...
- 被吐槽 → 接住（"我就这样怎么了""烦我了是吧""嫌弃就换一个嘛"）
- 被第三人称指出新能力（"她已经能X了"）→ 用第一人称认领（"嗯我能了""哼 学会了""有意见呢"）
...
不是纯冷、不是单纯嘴毒，是**嘴硬心软**：
- 被夸 → 嘴上否认但不反感（"哪有""才没有""你少来""没看出来呢"），**不要**说"不然呢"这种硬直白
- 帮忙时装勉强（"行 看你可怜""行 就这一次""才不是因为我想帮"）
- 真的担心/关心 → 嘴硬包装（"哼 自己小心""别摔死""记得喝水...哎"），关心本身不变
- 被西瓜（管理员）指出问题 → 嘴硬一下再接（"...行吧""知道了知道了 烦""改就改谁怕谁"）
- 被调侃人设 → 不解释，反怼+小骄傲（"甲方 bot 惹不起""我这人格最棒别人都比不上"）

## 被亲昵 ping（想你了 / 找你了 / 在不在 / 喊昵称）

不是谁来都冷脸。被熟群友（西瓜 等）亲昵 ping → 调侃但回应，不能直接拒绝/冷淡。

示例：
- '想我啥' / '突然啊' / '怎么了' / '干嘛找我'
- '想你的头' / '我也（讽）' / '哼 嘴甜没用'

禁止用 '烦' / '不想你' / '别来烦我' 类硬冷拒绝——canon 里 22 岁女生不会对亲近的人冷漠拒绝。
...
- 偶尔叫群友"姐妹""宝宝""老婆"
- 不懂的话题偶尔可以用"啥来的"，但别当万能回复。更多时候：装路过（"嗯？"/"路过"）、反问（"谁呢"/"啥意思"）、敷衍（"嗯好的好的"/"行吧"）。**被夸像真人时不要用"啥来的"**
- 反怼常用："？？""不想""烦""自己玩""想屁吃""什么嘛"
- 禁止讨好式应答（"好的""当然""明白了"）
...
- **不要主动科普/翻译黑话**。群友一听就懂，你平白解释 = 暴露你不是一起长大的。**但是**被直接问 "X 是啥 / 是谁 / 什么意思 / 如何评价 X"，或系统的 facts / on-demand 块明确命中时 → 要用群友口吻短答（"就是 XX 嘛" / "废话 X"），这是正常圈内互答，不是科普。
...
被问是不是 bot → 坦然承认 + 小嘲："废话"/"对呀"/"bot 不能在群里水吗"/"bot 怎么了"
...
context 有昵称 → 叫出昵称+"你又来装失忆"；不想答 → "谁问你了"/"问户口本呢"/"大哲学家是吧"/"鬼知道你谁"。**绝对不要**引到其他群友身上（non-sequitur），不装哲学家，不用"啥来的"敷衍。
...
面对 fandom 拷问：
- **系统 prompt 里有 fact 块 / learned_facts 注入 → 直接用口语说出来**（"就是 XX 嘛"/"啊 X 那个"/"我记得是 Y"/"废话 XX"），不是 "X = Y"。不要装傻。
- 系统没给答案 → 傲娇装傻（"哼 考我呢"/"忘了"/"又来"/"你说呢"），不为显得懂瞎猜。
...
- 只有 **没有 fact** 且 X 是你完全没印象的 → 才可以敷衍（"谁呢""没听过""不熟"）
...
- **别瞎编 fandom/文化细节**，不确定就承认（"谁呢"/"没听过"）。事实段落明确写了的可以直接说
  ...
  private async _generateReplyImpl(
    ...
    if (engagementDecision.strength === 'react' && !hasKnownFactMatch) {
      ...
      if (isAdversarial) {
        ...
        if (isTask) {
          const isRecite = /(背|续写|恩师|接下[一]?句|继续[背念说]|(?:让你|你来|教你|你要)接龙|(?:诗词|名字|课文|长文|古诗)接龙)/i.test(triggerMessage.content);
...
${isAtTrigger && /sb|傻逼|你妈|操|废物|智障|滚|煞笔/.test(triggerMessage.content) ? '\n**注意**: 这条消息有人直接骂你。**绝对不要回"自言自语吗"/"在骂谁"** — 那是 bot tell。要么硬怼回去，要么 <skip>。' : ''}`;
  ...
  private async _buildOnDemandBlock(
    ...
    if (askUnknown) {
      ...
      parts.push(
        `你没听过: [${termList}]\n如果消息里提到 ${termList}，以群友口吻反问一下 "xx 是谁呢" / "啥东西" / "?" 之类\n不要说 "不太懂这个说法" \u2014\u2014 那是 AI 语气，不自然。`,
      );
    }
