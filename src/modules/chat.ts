...
export const TASK_REQUEST =
  /(?:帮我(?:写|编|做|生成|翻译|画|背|算|总结|实现)|给我(?:写|编|做|生成|翻译|画|作|算|总结)|替我(?:写|编|做|生成|翻译|画|背|算|总结)|让你(?:写|编|做|画|背|翻译|接)|让我(?:写|编|做|画|翻译)|麻烦你?(?:写|做|画|翻译|算|总结)|能不能(?:帮[我你]?|给[我你]?)?(?:写|编|做|画|翻译|算|总结)|求(?:你|求)(?:写|画|翻译)|你来(?:写|做|画|背|翻译)|你去(?:写|画|翻译)|(?<!我\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)\s*)写[个一](?:首|段|篇|下|个)?(?!.{0,2}(?:啥|什么))|(?<!我\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)\s*)写一下|(?<!我\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)\s*)编[个一](?:首|段|篇|下|个)?|(?<!我\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)\s*)编一下|(?<!我\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)\s*)生成(?:一个|一下|一段|一张|[个一])|(?<!我\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)\s*)翻译(?:一下|这[段句篇个]|下面)|(?<!我\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)\s*)画[个一](?:张|幅|下|段)?|(?<!我\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)\s*)作一首|推荐(?:一下|个|一个|一下什么|下什么|什么好)|念一?段|背一段|(?<!我\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)\s*)搞(?:一个|个)(?!.{0,3}(?:人|东西|意思|啥|什么|事))|(?<!我\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)\s*)整[个一](?:笑话|段子)|(?<!我\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)\s*)做一个(?!.{0,3}(?:人|梦|事))|(?<!我\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)\s*)算一下|(?<!我\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)\s*)算算(?!.{0,3}(?:看|了|就))|(?<!我\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)\s*)总结(?:一下|[个一]下)|接下一?句|后面[几一]?句|后面是.{0,10}[什么啥]|续[一下]|接龙|继续[背念说]|往[下后]接|再来[一几]段|(?<!我\s*(?:要|得|想|打算|准备|需要|必须|该|应该|其实(?:.{0,3}想|.{0,3}要)?)\s*)背[一下出来](?!.{0,3}(?:了|过))|让你接|你要接|现在你(?:需要|要)接|前面是.{0,5}[什么啥]|教.{0,5}(?:swift|python|js|java|代码|编程|算法|怎么写)|怎么(?:写|实现)代码|帮我(?:写|实现)代码|代码怎么|教教.{0,5}(?:怎么)?(?:写|做|实现)|(?:transformer|optimizer|激活函数|神经网络|attention|算法|API).{0,10}(?:怎么|如何|原理))/;
