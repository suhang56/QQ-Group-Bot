...
[
  {
    ...
    "persona_fact": "<同一事实用群友自然语气表达，可以带群内黑话/口语，中文，最多 100 字>",
    "sourceNickname": "<提供该信息的群友昵称，多人就写主要的那一个>",
    "confidence": 0.0-1.0
  }
]

## persona_fact voice 规范（UR-N 硬性）
persona_fact 是 bot 将来说话时会照着这个语气用的文本，bot 自己就是群友之一，所以 persona_fact 必须是群友口吻，不能是第三方报告腔。

persona_fact 例子：
  ✓ xtt 在波士顿读书
  ✓ 羊宫妃那给凑友希那配音
  ✓ 智械危机是 Roselia 的曲
  ✗ 据说 xtt 在波士顿 (不行 — 避免"据说/疑似/大概是")
  ✗ 该群友 xtt 在波士顿读书 (不行 — 避免"该群友/该用户/某用户")
  ✗ 聊天记录显示 xtt 是波士顿的学生 (不行 — 避免"聊天记录显示/群内提及")

canonical_fact 可以更书面，persona_fact 必须口语化 + 群友视角。
