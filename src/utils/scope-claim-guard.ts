...
//
// Tail addressee (`你们/大家/你俩/诸位`) and optional `是吧` modal sit BEFORE the
// existing particle/punct tail. Covers archived 04-23/04-24 live samples where
// bot ate generic 你们 rhetoric and emitted "又来了你们" / "又来这套是吧你们" /
// "又怎么了你们". Semantically self-centered ("bot feels targeted") but shape-
// wise distinct from Group A (which owns 你们-PREFIX patterns).
//
// Independence invariant: PLURAL_YOU_PATTERNS owns 你们-prefix; this group owns
// trigger-then-(modal)-(addressee). The two never co-fire on any single
// compact string.
export const SELF_CENTERED_SCOPE_CLAIM_PATTERNS: readonly RegExp[] = [
  /^(?:又来了|又开始了|又来搞我|又在搞我|还来|又一次|有完没完|又来这套|又怎么了)(?:是吧)?(?:你们|大家|你俩|诸位)?[啊了呢吧哦嗷哈]*[。.!?~～！？]*$/,
];
