#!/usr/bin/env tsx
/**
 * R2.5.1-annex — purge vulgar-dismissal learned_facts.
 *
 * Flips rows in `learned_facts` whose `topic` is one of the auto-ingest
 * prefixes (`ondemand-lookup:%` / `opus-classified:slang:%` /
 * `opus-rest-classified:slang:%`) AND whose topic-suffix term matches
 * `isVulgarDismissal` to `status='rejected'`. No DELETE — the
 * `learned_facts_au` trigger keeps the paired FTS5 mirror in sync. Lore-topic
 * rows (topic contains `lore:`) are exempt — they are fandom-curated.
 *
 * Usage:
 *   purge-vulgar-phrase-facts.ts --db-path <path> [--apply] [--verbose]
...
/**
 * Auto-ingest prefixes ONLY (ondemand-lookup + opus-classified:slang +
 * opus-rest-classified:slang). Lore-topic rows filtered out via NOT LIKE.
 * `user-taught:*` and `群内黑话:*` are NEVER touched — curated sources stay
 * authoritative even if the canonical looks vulgar.
 */
const SELECT_SQL = `
...
export function runPurge(opts: {
  ...
  const matched: Row[] = [];
  let skippedNonVulgar = 0;
  for (const r of candidates) {
    const term = extractTermFromTopic(r.topic);
    if (term !== null && isVulgarDismissal(term)) {
      matched.push(r);
    } else {
      skippedNonVulgar++;
    }
  ...
  log(heading);
  log(`  learned_facts vulgar-dismissal rows: ${matched.length} found, ${opts.apply ? updated : 0} ${action}`);
  log(`  skipped (lore-topic exempt): ${loreSkipped}`);
  log(`  skipped (non-vulgar canonical): ${skippedNonVulgar}`);
  ...
  return {
    ...
    skippedNonVulgarCount: skippedNonVulgar,
    matched,
  };
}
...
function main(argv: ReadonlyArray<string>): number {
  ...
  if (!parsed) {
    console.error('Usage: purge-vulgar-phrase-facts.ts --db-path <path> [--apply] [--verbose]');
    return 2;
  }
...
// Only auto-run when executed directly. Mirrors purge-social-phrase-facts.ts.
